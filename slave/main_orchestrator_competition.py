...
MAVSDK_ADDRESS       = os.environ.get("MAVSDK_ADDR", "udp://:14540")
    ...
    async def _start_telem(self):
        async def _pos():
            async for p in self.drone.telemetry.position():
                ...
                self.current_alt = p.relative_altitude_m
...
#  TCP COMMAND SERVER
# ─────────────────────────────────────────────────────────────────────────────
def start_tcp_command_server(movement):
    ...
    def h_goto(cmd):
        ...
        asyncio.run_coroutine_threadsafe(
            movement.move_to(lat, lon, alt), asyncio.get_running_loop())
        return {"status": "ok"}
    ...
    def h_pause(cmd):
        ...
        asyncio.run_coroutine_threadsafe(
            movement.force_hover(0), asyncio.get_running_loop())
    ...
    def h_land(cmd):
        ...
        asyncio.run_coroutine_threadsafe(
            movement.land(), asyncio.get_running_loop())
    ...
    def h_side_move(cmd):
        ...
        if _side_move_event is not None:
            asyncio.run_coroutine_threadsafe(
                _trigger_side_move(), asyncio.get_running_loop())
    ...
    def h_arm_takeoff(cmd):
        ...
        asyncio.run_coroutine_threadsafe(
            movement.takeoff_to_hover(alt), asyncio.get_running_loop())
        return {"status": "ok"}

    @server.on_command("ARM_ONLY")
    def h_arm_only(cmd):
        async def _a(): await movement.drone.action.arm()
        asyncio.run_coroutine_threadsafe(_a(), asyncio.get_running_loop())
        return {"status": "ok"}

    @server.on_command("DISARM")
    def h_disarm(cmd):
        async def _d(): await movement.drone.action.disarm()
        asyncio.run_coroutine_threadsafe(_d(), asyncio.get_running_loop())
        return {"status": "ok"}
...
async def main():
    ...
    try:
        start_tcp_command_server(movement)
    except Exception as e:
