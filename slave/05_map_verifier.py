#!/usr/bin/env python3
"""
05_map_verifier.py  —  Mine Deduplication & Map Logger
======================================================
Pipeline step 5 of 6.  Called AFTER 04_coordinate_math confirms a GPS
position for a new mine.  Does two things:

1. DEDUPLICATION
   Before broadcasting the mine to the master, check whether the drone
   already found and reported this same mine earlier in the same flight
   (e.g. saw it twice on two adjacent scan passes).
   If it's within DEDUPLICATION_RADIUS_M of an already-logged mine:
     → Average the coordinates (coordinate fusion → higher accuracy)
     → Mark confidence = 1.0 (confirmed by two independent scans)
     → Return False  (don't send duplicate report to master)

2. INTEL CHECK + LOGGING
   Compare against the pre-known buried mines from the A4 survey map
   (config/A4_map.json, generated at startup from fieldmap.py).
   • If it matches a known mine → type = "KNOWN_MINE"
   • If it's a new discovery   → type = "NEW_DISCOVERY"
   Either way, append to logs/virtual_map.json and return True
   (signal to orchestrator: send this mine report to master).

Files used
----------
  config/A4_map.json        — pre-known buried mine positions (generated at startup)
  logs/virtual_map.json     — running log of all mines found this flight (auto-created)

Called from
-----------
    main_orchestrator_competition.py → _handle_candidate() → _report():
        if verifier.verify_and_log(tlat, tlon, final_conf):
            tunnel.send_anomaly_data(tlat, tlon, restack)   # → master
"""
import json
import math
import os
