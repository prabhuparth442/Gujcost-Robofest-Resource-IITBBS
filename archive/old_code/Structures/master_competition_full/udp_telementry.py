...
# ════════════════════════════════════════════════════════════════════════════
#  FIX 6 — MINE TCP LISTENER
#  Receives DroneTunnel anomaly_report packets from slaves (4-byte len + JSON).
#  Calls STATE.add_mine() then POSTs to app.py so the phone map shows the mine.
# ════════════════════════════════════════════════════════════════════════════
MINE_TCP_PORT = 5000   # must match DroneTunnel target_port in 08_comms_link.py
...
async def _handle_mine_connection(
    ...
    """
    Handles one incoming DroneTunnel connection from a slave.
    DroneTunnel wire format: 4-byte big-endian length prefix, then JSON body.
    """
    ...
    try:
        ...
        if payload_len > 5_000_000:
            log.warning(f"[MINE TCP] Oversized packet {payload_len}B from {peer} — dropping")
            return
        ...
        if pkt_type == "anomaly_report":
            ...
            mine = await STATE.add_mine(lat, lng, detected_by=drone_id)
            log.info(f"[MINE TCP] Added to STATE as mine #{mine.mine_id}")

            # POST to app.py so phone map and A* path planner see it
            asyncio.create_task(
                _post_to_ground({
                    "type":     "mine_detected",
                    "lat":      lat,
            ...
            log.info(f"[MINE TCP] Forwarded to app.py /api/drone_update")

        elif pkt_type == "sector_result":
            # Sector result also carries confirmed mine GPS when mine_found=True
            ...
            if found:
                ...
                if mlat and mlng:
                    mine = await STATE.add_mine(float(mlat), float(mlng), detected_by=drone_id)
                    asyncio.create_task(
                        _post_to_ground({
                            "type":     "mine_detected",
                            "lat":      mlat,
...
async def _mine_tcp_server() -> None:
    """Runs forever — accepts DroneTunnel connections from all slaves."""
...
async def start_udp_server() -> None:
    ...
    # UDP endpoint — ONE socket, all drones send here
    ...
    # Mine TCP listener — receives DroneTunnel anomaly_report + sector_result
