...
# ════════════════════════════════════════════════════════════════════════════
#  MASTER GRID  — accumulated coverage from all slaves
# ════════════════════════════════════════════════════════════════════════════
try:
...
# ════════════════════════════════════════════════════════════════════════════
#  FIELD MAP  — pre-known hazards (competition edition)
# ════════════════════════════════════════════════════════════════════════════
try:
...
# ════════════════════════════════════════════════════════════════════════════
#  A* PATH PLANNER  (field-aware)
# ════════════════════════════════════════════════════════════════════════════
STEP        = 0.5
SAFE_RADIUS = 1.0
FIELD_W     = 32.0   # metres East limit (field extends to ~28 m)
FIELD_S     = 24.0   # metres South limit (field extends to ~19 m South)
FIELD_L     = 100.0  # legacy forward distance kept for /api/get_my_location


def _all_hazard_circles(runtime_mines: list[dict]) -> list[dict]:
    """Merge pre-known buried mines, forbidden zones, and runtime detections."""
    circles = []
    if _FIELD_AWARE:
...
def calculate_path(start_x, start_y, goal_y, mines):
    # Build combined hazard list including pre-known objects
...
def _build_status() -> dict:
    with _lock:
        ...
        # Grid coverage — only cells the drones have actually scanned
        covered_cells = []
        coverage_pct  = 0.0
        ...
        return {
            ...
            "covered_cells":   covered_cells,    # only visited cells
...
def drone_update():
    ...
    # Grid merge runs outside the main lock (GridMap has its own lock)
    if _GRID_AVAILABLE and _master_grid is not None:
